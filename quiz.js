...
confirmSubmitBtn.addEventListener('click', () => { confirmModal.classList.add('hidden'); finalizeAndSubmit(); });
...
function startQuiz(){
  ...
  if(!name){ showStartStatus('Please enter your name.'); return; }
  if(!gmailRegex.test(email)){ showStartStatus('Please enter a valid Gmail (ends with @gmail.com).'); return; }
...
function updateTimerDisplay(){ timerEl.textContent = '⏱️ ' + timeLeft + 's'; }

function onNext(){
  const sel = quizContainer.querySelector('input[name="choice"]:checked');
...
function updateProgressUI(){
  ...
  progressText.textContent = 'Answered ' + answered + ' / ' + questions.length;
}
...
function finalizeAndSubmit(){
  ...
  finishQuiz();
  try{
    const payload = { name: studentName, email: studentEmail, answers, timestamp: new Date().toISOString() };
...
function showToast(msg){ if(!toast) return; toast.textContent = msg; toast.classList.remove('hidden'); setTimeout(()=>{ toast.classList.add('hidden'); }, 3500); }
function clearToast(){ if(!toast) return; toast.classList.add('hidden'); toast.textContent = ''; }
